...
docker build -t {REGION}-docker.pkg.dev/{PROJECT_ID}/microservices_with_go/api-gateway:latest --platform linux/amd64 -f infra/production/docker/api-gateway.Dockerfile .

# Build the Driver service 
docker build -t {REGION}-docker.pkg.dev/{PROJECT_ID}/microservices_with_go/driver-service:latest --platform linux/amd64 -f infra/production/docker/driver-service.Dockerfile .

# Build the Trip service 
docker build -t {REGION}-docker.pkg.dev/{PROJECT_ID}/microservices_with_go/trip-service:latest --platform linux/amd64 -f infra/production/docker/trip-service.Dockerfile .

# Build the Payment service 
docker build -t {REGION}-docker.pkg.dev/{PROJECT_ID}/microservices_with_go/payment-service:latest --platform linux/amd64 -f infra/production/docker/payment-service.Dockerfile .
```
...
```bash
gcloud container clusters get-credentials microservices_with_go --region {REGION}--project {PROJECT_ID}
```
